# 👋 Hi, I'm Qadeer Ahmed

![Banner Image](https://github.com/qadeer884/qadeer884/blob/main/Navy%20Blue%20Geometric%20Technology%20LinkedIn%20Banner%20(2).png)

I'm an **AI/ML Developer** with hands-on experience in machine learning, deep learning, NLP, and computer vision. I have worked on multiple real-world projects – from fine-tuning models for classification and object detection to developing AI-powered web applications.

## 🧑‍💻 About Me

- 🎓 Graduated with a BS in Information Technology from the University of Chakwal (2020–2024) with a CGPA of 3.39.
- 🤖 Experienced in building, training, and deploying AI/ML models, including work with CNNs, YOLO, and conversational chatbots.
- 🚀 Passionate about developing practical AI solutions, with internships and projects at Innovista Rawal, Gelecek, and Bits Phile.
- 💡 Always eager to explore new technologies and optimize AI models for real-world applications.

## 🛠️ Technologies & Tools

### Programming & Scripting
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="Python" width="40" height="40"/>
</p>

### AI/ML Frameworks & Libraries
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/tensorflow/tensorflow-original.svg" alt="TensorFlow" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pytorch/pytorch-original.svg" alt="PyTorch" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/scikit-learn/scikit-learn/main/doc/logos/scikit-learn-logo-notext.png" alt="scikit-learn" width="40" height="40"/>
  <img src="https://upload.wikimedia.org/wikipedia/commons/8/87/Keras_logo_small_icon.png" alt="Keras" width="40" height="40"/>
  <img src="https://avatars.githubusercontent.com/u/8497296?s=200&v=4" alt="LangChain" width="40" height="40"/>
</p>

### Data Visualization & Analysis
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pandas/pandas-original.svg" alt="Pandas" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/numpy/numpy-original.svg" alt="NumPy" width="40" height="40"/>
  <img src="https://upload.wikimedia.org/wikipedia/commons/8/84/Matplotlib_icon.svg" alt="Matplotlib" width="40" height="40"/>
</p>

### Tools & Version Control
<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" alt="Git" width="40" height="40"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" alt="GitHub" width="40" height="40"/>
</p>

### Additional Tools
- **API Development & Testing**: Postman
- **System Control**: Systemctl
- **Data Visualization**: Power BI

## 🚀 Experience

### AI/ML Professional Trainee
- **Innovista Rawal** | Rawalpindi, Pakistan (Nov 2024 – Mar 2025)
  - Built skills in data science, machine learning, deep learning, NLP, and computer vision while contributing to market-driven AI solutions.

### AI/ML Internship
- **Gelecek** | Rawalpindi, Pakistan (Dec 2024 – Mar 2025)
  - Worked on data analysis, NLP (chatbots, RAG), and computer vision (YOLO model training).
  - Gained experience with model optimization, API development, and testing with Postman.

### AI/ML Intern
- **Bits Phile** | Chakwal, Pakistan (Aug 2024 – Oct 2024)
  - Contributed to exploratory data analysis, model development, evaluation, and data visualization.

## 🔥 Featured Project: Interview.AI

During my final year project, I developed **Interview.AI**—an AI-based web application that assesses interview candidates' personalities using multimodal data (visual, audio, and text). The project involved:
- End-to-end AI model training, from data collection and preprocessing to model evaluation.
- Frontend integration to present real-time assessments.

## 🏗️ Projects

Some of my notable projects include:
- **Interview.AI**: An AI-powered web app for personality assessment.
- **Object Detection Projects**: YOLO-based fruit detection and CNN for plant classification.
- **NLP Applications**: Chatbots and text summarization projects using transfer learning and model fine-tuning.

## 📈 GitHub Stats

![Qadeer's GitHub Stats](https://github-readme-stats.vercel.app/api?username=qadeer884&show_icons=true&theme=radical)

## 🌱 What I'm Learning

- Advanced **Computer Vision** and **NLP** techniques
- Further optimization of deep learning architectures and AI model deployment

## 📫 How to Reach Me

- **LinkedIn**: [Qadeer Ahmed](https://www.linkedin.com/in/qadeer84)
- **Email**: qadeer.3884@gmail.com

---

⭐️ Feel free to explore my repositories and projects. I'm always open to collaboration and new opportunities!
